...
# Tentukan CORS Origin
CORS_ORIGINS = os.environ.get('CORS_ORIGINS', 'http://localhost:5173')
CORS(app, supports_credentials=True, origins=CORS_ORIGINS) 

# HACK CORS: Memaksa header credentials
@app.after_request
def after_request(response):
    header = response.headers
    header['Access-Control-Allow-Origin'] = os.environ.get('CORS_ORIGINS', 'http://localhost:5173')
...
DATABASE_URL = os.environ.get(
    "DATABASE_URL", 
    "postgresql+psycopg2://postgres:Admin123@localhost:5432/pasokari_db" # Fallback lokal
)

# Render menggunakan skema 'postgres://', ubah ke format yang dibutuhkan SQLAlchemy
...
# --- 3. DEFINISI MODEL DATABASE & USER HARDCODED FIX ---

# Model Admin (User) Hardcoded untuk Flask-Login (Di dalam memori)
class User(UserMixin):
    def __init__(self, id, email, password_hash):
        self.id = id
        self.email = email
        self.password_hash = password_hash
    def to_dict(self): return {'id': self.id, 'email': self.email}

# Data Admin (Gunakan hash yang terjamin berfungsi)
...
def load_user(user_id):
    # Mengambil user dari dictionary hardcoded
...
# --- 5. FUNGSI SEEDING DATA (DIPINDAHKAN KE TOP-LEVEL) ---
def seed_database():
    """Membuat tabel dan mengisi data awal (produk) jika database kosong."""
    try:
        with app.app_context():
            # Membuat semua tabel
            db.create_all()

            # Seeding Produk (jika kosong)
            if Product.query.count() == 0:
                print("INFO: Seeding data produk...")
                product_list = []
                for category, data in CATEGORY_DATA.items():
                    # Asumsi CATEGORY_DATA memiliki struktur: {'Category': {'id': {'products': [...]}, 'en': {'products': [...]}}}
                    products_id = data['id']['products']
                    products_en = data['en']['products']
                    
                    for name_id, name_en in zip(products_id, products_en):
                        new_product = Product(category=category, name_id=name_id, name_en=name_en)
                        product_list.append(new_product)
                
                db.session.bulk_save_objects(product_list)
                db.session.commit()
                print(f"INFO: Products seeded ({len(product_list)} items).")
            else:
...
def login():
    if current_user.is_authenticated: return jsonify({"message": "User sudah login.", "user": current_user.to_dict()}), 200
    ...
    # Cari user di dictionary hardcoded
    user_found = users.get(1) if email == users.get(1).email else None 
    ...
    if user_found and check_password_hash(user_found.password_hash, password):
        ...
        return jsonify({"message": "Login berhasil.", "user": user_found.to_dict()}), 200
    else: return jsonify({"message": "Email atau Password salah."}), 401

@app.route('/api/logout', methods=['POST'])
@login_required
def logout():
    logout_user(); print(f"Logout sukses: {current_user.email}")
...
def add_product():
    ...
    except Exception as e: db.session.rollback(); print(f"Error POST product: {e}"); return jsonify({"message": f"Gagal tambah produk. Error: {e}"}), 500
...
if __name__ == '__main__':
    # Gunicorn akan memanggil app.run() secara otomatis tanpa seed_database() di sini
    app.run(debug=True)